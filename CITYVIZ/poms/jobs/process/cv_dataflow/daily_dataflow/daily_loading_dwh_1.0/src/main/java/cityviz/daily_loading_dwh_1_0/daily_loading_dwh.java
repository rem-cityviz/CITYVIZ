...
package cityviz.daily_loading_dwh_1_0;
...
/**
 * Job: daily_loading_dwh Purpose: Alimentation quotidienne du dwh depuis notre
 * base de staging.<br>
 * Description: 1.1 : Premier essais du job permettant d'alimenter notre DWH
 * avec les nouveaux marchés publiés le jour même. <br>
 * 
 * @author user@talend.com
 * @version 8.0.1.20211109_1610
 * @status
 */
public class daily_loading_dwh implements TalendJob {
	...
	private final String jobName = "daily_loading_dwh";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					daily_loading_dwh.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(daily_loading_dwh.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row10Struct implements routines.system.IPersistableRow<row10Struct> {
		final static byte[] commonByteArrayLock_CITYVIZ_daily_loading_dwh = new byte[0];
		static byte[] commonByteArray_CITYVIZ_daily_loading_dwh = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_CITYVIZ_daily_loading_dwh.length) {
					if (length < 1024 && commonByteArray_CITYVIZ_daily_loading_dwh.length == 0) {
						commonByteArray_CITYVIZ_daily_loading_dwh = new byte[1024];
					} else {
						commonByteArray_CITYVIZ_daily_loading_dwh = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_CITYVIZ_daily_loading_dwh, 0, length);
				strReturn = new String(commonByteArray_CITYVIZ_daily_loading_dwh, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_CITYVIZ_daily_loading_dwh, 0, length);
				strReturn = new String(commonByteArray_CITYVIZ_daily_loading_dwh, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_CITYVIZ_daily_loading_dwh) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				try {
					...
					Object filename_tFileInputDelimited_1 = context.logs_path + "/2022_daily_staging_loading/"
							+ ((int) TalendDate.getPartOfDate("MONTH", TalendDate.getCurrentDate()) + 1) + "/"
							+ TalendDate.getPartOfDate("DAY_OF_MONTH", TalendDate.getCurrentDate()) + "_to_dwh.csv";
					...
					try {
						fid_tFileInputDelimited_1 = new org.talend.fileprocess.FileInputDelimited(
								context.logs_path + "/2022_daily_staging_loading/"
										+ ((int) TalendDate.getPartOfDate("MONTH", TalendDate.getCurrentDate()) + 1)
					...
					while (fid_tFileInputDelimited_1 != null && fid_tFileInputDelimited_1.nextRecord()) {
						...
						try {
							...
							row11.recordid = fid_tFileInputDelimited_1.get(columnIndexWithD_tFileInputDelimited_1);
...
// Start of branch "row11"
						if (row11 != null) {
							...
							globalMap.put("id_to_dwh", row11.recordid);
							...
							if (execStat) {
								runStat.updateStatOnConnection("etablissement", 3, 0);
							}

							if (execStat) {
								runStat.updateStatOnConnection("doublon_etablissement", 3, 0);
							}

							if (execStat) {
								runStat.updateStatOnConnection("doublons_acheteur", 3, 0);
							}

							if (execStat) {
								runStat.updateStatOnConnection("dim_etablissement", 3, 0);
							}

							if (execStat) {
								runStat.updateStatOnConnection("siren_null", 3, 0);
							}
							...
							if (execStat) {
								runStat.updateStatOnConnection("doublon_marche", 3, 0);
							}

							if (execStat) {
								runStat.updateStatOnConnection("dim_acheteur", 3, 0);
							}
							...
							if (execStat) {
								runStat.updateStatOnConnection("acheteur", 3, 0);
							}

							if (execStat) {
								runStat.updateStatOnConnection("dim_marche", 3, 0);
							}

							if (execStat) {
								runStat.updateStatOnConnection("marche", 3, 0);
							}
							...
							fileName_tFileOutputDelimited_1 = (new java.io.File(context.logs_path
									+ "/2022_daily_dwh_loading/"
									+ ((int) TalendDate.getPartOfDate("MONTH", TalendDate.getCurrentDate()) + 1) + "/"
									+ TalendDate.getPartOfDate("DAY_OF_MONTH", TalendDate.getCurrentDate())
									+ "_doublons_dim_acheteur.csv")).getAbsolutePath().replace("\\", "/");
...
// ###############################
							...
							fileName_tFileOutputDelimited_2 = (new java.io.File(context.logs_path
									+ "/2022_daily_dwh_loading/"
									+ ((int) TalendDate.getPartOfDate("MONTH", TalendDate.getCurrentDate()) + 1) + "/"
									+ TalendDate.getPartOfDate("DAY_OF_MONTH", TalendDate.getCurrentDate())
									+ "_doublons_dim_marche.csv")).getAbsolutePath().replace("\\", "/");
							...
							outtFileOutputDelimited_2 = new java.io.BufferedWriter(new java.io.OutputStreamWriter(
									new java.io.FileOutputStream(fileName_tFileOutputDelimited_2, false),
									"ISO-8859-15"));
...
// ###############################
							...
							fileName_tFileOutputDelimited_4 = (new java.io.File(
									context.data_storage_path + "/dim_etablissement.csv")).getAbsolutePath()
											.replace("\\", "/");
							...
							outtFileOutputDelimited_4 = new java.io.BufferedWriter(new java.io.OutputStreamWriter(
									new java.io.FileOutputStream(fileName_tFileOutputDelimited_4, false),
									"ISO-8859-15"));
							if (filetFileOutputDelimited_4.length() == 0) {
								outtFileOutputDelimited_4.write("sirenetablissement");
								outtFileOutputDelimited_4.write(OUT_DELIM_tFileOutputDelimited_4);
								outtFileOutputDelimited_4.write("siretetablissement");
								outtFileOutputDelimited_4.write(OUT_DELIM_tFileOutputDelimited_4);
								outtFileOutputDelimited_4.write("siretetablissementvalide");
								outtFileOutputDelimited_4.write(OUT_DELIM_tFileOutputDelimited_4);
								outtFileOutputDelimited_4.write("categorieetablissement");
								outtFileOutputDelimited_4.write(OUT_DELIM_tFileOutputDelimited_4);
								outtFileOutputDelimited_4.write("denominationsocialeetablissement");
								outtFileOutputDelimited_4.write(OUT_DELIM_tFileOutputDelimited_4);
								outtFileOutputDelimited_4.write("codecommuneetablissement");
								outtFileOutputDelimited_4.write(OUT_DELIM_tFileOutputDelimited_4);
								outtFileOutputDelimited_4.write("communeetablissement");
								outtFileOutputDelimited_4.write(OUT_DELIM_tFileOutputDelimited_4);
								outtFileOutputDelimited_4.write("departementetablissement");
								outtFileOutputDelimited_4.write(OUT_DELIM_tFileOutputDelimited_4);
								outtFileOutputDelimited_4.write("libelledepartementetablissement");
								outtFileOutputDelimited_4.write(OUT_DELIM_tFileOutputDelimited_4);
								outtFileOutputDelimited_4.write("coderegionetablissement");
								outtFileOutputDelimited_4.write(OUT_DELIM_tFileOutputDelimited_4);
								outtFileOutputDelimited_4.write("libelleregionetablissement");
								outtFileOutputDelimited_4.write(OUT_DELIM_tFileOutputDelimited_4);
								outtFileOutputDelimited_4.write("codenaf");
							...
							fileName_tFileOutputDelimited_3 = (new java.io.File(context.logs_path
									+ "/2022_daily_dwh_loading/"
									+ ((int) TalendDate.getPartOfDate("MONTH", TalendDate.getCurrentDate()) + 1) + "/"
									+ TalendDate.getPartOfDate("DAY_OF_MONTH", TalendDate.getCurrentDate())
									+ "_doublons_dim_etablissement.csv")).getAbsolutePath().replace("\\", "/");
							...
							outtFileOutputDelimited_3 = new java.io.BufferedWriter(new java.io.OutputStreamWriter(
									new java.io.FileOutputStream(fileName_tFileOutputDelimited_3, false),
									"ISO-8859-15"));
							...
							fileName_tFileOutputDelimited_5 = (new java.io.File(context.logs_path
									+ "/2022_daily_dwh_loading/"
									+ ((int) TalendDate.getPartOfDate("MONTH", TalendDate.getCurrentDate()) + 1) + "/"
									+ TalendDate.getPartOfDate("DAY_OF_MONTH", TalendDate.getCurrentDate())
									+ "_siren_null_dim_etablissement.csv")).getAbsolutePath().replace("\\", "/");
							...
							outtFileOutputDelimited_5 = new java.io.BufferedWriter(new java.io.OutputStreamWriter(
									new java.io.FileOutputStream(fileName_tFileOutputDelimited_5, false),
									"ISO-8859-15"));
...
// ###############################
							...
							String dbquery_tDBInput_1 = "SELECT * FROM \"" + context.cityviz_staging_Schema
									+ "\".\"marche_v2\" WHERE \"" + context.cityviz_staging_Schema
									+ "\".\"marche_v2\".\"recordid\" = '" + ((String) globalMap.get("id_to_dwh")) + "'";
							...
							try {
								...
								while (rs_tDBInput_1.next()) {
									...
									if (colQtyInRs_tDBInput_1 < 2) {
										...
										row1.datepublicationdonnees = routines.system.JDBCUtil.getDate(rs_tDBInput_1,
												2);
									}
									...
									if (colQtyInRs_tDBInput_1 < 8) {
										...
										row1.siretetablissement = routines.system.JDBCUtil.getString(rs_tDBInput_1, 8,
												false);
									}
									...
									if (colQtyInRs_tDBInput_1 < 11) {
										...
										row1.codecommuneacheteur = routines.system.JDBCUtil.getString(rs_tDBInput_1, 11,
												false);
									}
									...
									if (colQtyInRs_tDBInput_1 < 13) {
										...
										row1.lieuexecutionnom = routines.system.JDBCUtil.getString(rs_tDBInput_1, 13,
												false);
									}
									...
									if (colQtyInRs_tDBInput_1 < 16) {
										...
										row1.sirenetablissement = routines.system.JDBCUtil.getString(rs_tDBInput_1, 16,
												false);
									}
									if (colQtyInRs_tDBInput_1 < 17) {
										...
										row1.codetypeetablissement = routines.system.JDBCUtil.getString(rs_tDBInput_1,
												17, false);
									}
									if (colQtyInRs_tDBInput_1 < 18) {
										...
										row1.nicetablissement = routines.system.JDBCUtil.getString(rs_tDBInput_1, 18,
												false);
									}
									if (colQtyInRs_tDBInput_1 < 19) {
										...
										row1.lieuexecutiontypecode = routines.system.JDBCUtil.getString(rs_tDBInput_1,
												19, false);
									}
									if (colQtyInRs_tDBInput_1 < 20) {
										...
										row1.libelleregionetablissement = routines.system.JDBCUtil
									...
									if (colQtyInRs_tDBInput_1 < 21) {
										...
										row1.categorieetablissement = routines.system.JDBCUtil.getString(rs_tDBInput_1,
												21, false);
									}
									...
									if (colQtyInRs_tDBInput_1 < 24) {
										...
										row1.communeetablissement = routines.system.JDBCUtil.getString(rs_tDBInput_1,
												24, false);
									}
									if (colQtyInRs_tDBInput_1 < 25) {
										...
										row1.libellecommuneacheteur = routines.system.JDBCUtil.getString(rs_tDBInput_1,
												25, false);
									}
									if (colQtyInRs_tDBInput_1 < 26) {
										...
										row1.libelledepartementetablissement = routines.system.JDBCUtil
												.getString(rs_tDBInput_1, 26, false);
									}
									if (colQtyInRs_tDBInput_1 < 27) {
										row1.dureemoisestimee = null;
									} else {

										row1.dureemoisestimee = routines.system.JDBCUtil.getString(rs_tDBInput_1, 27,
												false);
									}
									if (colQtyInRs_tDBInput_1 < 28) {
										...
										row1.libelleregionacheteur = routines.system.JDBCUtil.getString(rs_tDBInput_1,
												28, false);
									}
									if (colQtyInRs_tDBInput_1 < 29) {
										...
										row1.coderegionacheteur = routines.system.JDBCUtil.getString(rs_tDBInput_1, 29,
												false);
									}
									if (colQtyInRs_tDBInput_1 < 30) {
										...
										row1.siretetablissementvalide = routines.system.JDBCUtil
									...
									if (colQtyInRs_tDBInput_1 < 31) {
										...
										row1.codecommuneetablissement = routines.system.JDBCUtil
												.getString(rs_tDBInput_1, 31, false);
									}
									if (colQtyInRs_tDBInput_1 < 32) {
										row1.codepostalacheteur = null;
									} else {

										row1.codepostalacheteur = routines.system.JDBCUtil.getString(rs_tDBInput_1, 32,
												false);
									}
									if (colQtyInRs_tDBInput_1 < 33) {
										...
										row1.departementetablissement = routines.system.JDBCUtil
									...
									if (colQtyInRs_tDBInput_1 < 34) {
										...
										row1.superficiecommuneacheteur = routines.system.JDBCUtil
												.getString(rs_tDBInput_1, 34, false);
									}
									...
									if (colQtyInRs_tDBInput_1 < 37) {
										...
										row1.sirenacheteurvalide = routines.system.JDBCUtil.getString(rs_tDBInput_1, 37,
												false);
									}
									...
									if (colQtyInRs_tDBInput_1 < 39) {
										...
										row1.sirenetablissementvalide = routines.system.JDBCUtil
												.getString(rs_tDBInput_1, 39, false);
									}
									...
									if (colQtyInRs_tDBInput_1 < 43) {
										...
										row1.denominationsocialeetablissement = routines.system.JDBCUtil
												.getString(rs_tDBInput_1, 43, false);
									}
									...
									if (colQtyInRs_tDBInput_1 < 45) {
										...
										row1.lieuexecutioncode = routines.system.JDBCUtil.getString(rs_tDBInput_1, 45,
												false);
									}
									if (colQtyInRs_tDBInput_1 < 46) {
										...
										row1.moisnotification = routines.system.JDBCUtil.getString(rs_tDBInput_1, 46,
												false);
									}
									if (colQtyInRs_tDBInput_1 < 47) {
										...
										row1.codecpv_original = routines.system.JDBCUtil.getString(rs_tDBInput_1, 47,
												false);
									}
									if (colQtyInRs_tDBInput_1 < 48) {
										...
										row1.typeidentifiantetablissement = routines.system.JDBCUtil
									...
									if (colQtyInRs_tDBInput_1 < 50) {
										...
										row1.libellearrondissementacheteur = routines.system.JDBCUtil
												.getString(rs_tDBInput_1, 50, false);
									}
									...
									if (colQtyInRs_tDBInput_1 < 52) {
										...
										row1.adresseetablissement = routines.system.JDBCUtil.getString(rs_tDBInput_1,
												52, false);
									}
									...
									if (colQtyInRs_tDBInput_1 < 55) {
										...
										row1.natureobjetmarche = routines.system.JDBCUtil.getString(rs_tDBInput_1, 55,
												false);
									}
									...
									if (colQtyInRs_tDBInput_1 < 57) {
										...
										row1.anneenotification = routines.system.JDBCUtil.getString(rs_tDBInput_1, 57,
												false);
									}
									if (colQtyInRs_tDBInput_1 < 58) {
										...
										row1.libelledepartementacheteur = routines.system.JDBCUtil
												.getString(rs_tDBInput_1, 58, false);
									}
									if (colQtyInRs_tDBInput_1 < 59) {
										row1.departementacheteur = null;
									} else {

										row1.departementacheteur = routines.system.JDBCUtil.getString(rs_tDBInput_1, 59,
												false);
									}
									if (colQtyInRs_tDBInput_1 < 60) {
										...
										row1.codepostaletablissement = routines.system.JDBCUtil.getString(rs_tDBInput_1,
												60, false);
									}
									...
									if (colQtyInRs_tDBInput_1 < 64) {
										...
										row1.referencecpv = routines.system.JDBCUtil.getString(rs_tDBInput_1, 64,
												false);
									}
									if (colQtyInRs_tDBInput_1 < 65) {
										...
										row1.libelleregionexecution = routines.system.JDBCUtil.getString(rs_tDBInput_1,
												65, false);
									}
...
// Start of branch "dim_acheteur"
									if (dim_acheteur != null) {
										...
										if (tHash_Lookup_row2 != null && tHash_Lookup_row2.getCount(row2HashKey) > 1) { // G
											...
											// 'row2' and it contains more one result from keys : row2.id = '" +
											// row2HashKey.id + "'");
										} // G 071
...
// Start of branch "acheteur"
										if (acheteur != null) {
											...
											if (acheteur.idacheteur == null) {
												pstmtUpdate_tDBOutput_1.setNull(9 + count_tDBOutput_1,
														java.sql.Types.VARCHAR);
											} else {
												pstmtUpdate_tDBOutput_1.setString(9 + count_tDBOutput_1,
														acheteur.idacheteur);
											}
...
// Start of branch "dim_marche"
									if (dim_marche != null) {
										...
										if (tHash_Lookup_row3 != null && tHash_Lookup_row3.getCount(row3HashKey) > 1) { // G
											...
											// System.out.println("WARNING: UNIQUE MATCH is configured for the lookup
											// 'row3' and it contains more one result from keys : row3.id = '" +
											// row3HashKey.id + "'");
										} // G 071
...
// Start of branch "marche"
										if (marche != null) {
											...
											if (marche.idmarche == null) {
												pstmtUpdate_tDBOutput_2.setNull(17 + count_tDBOutput_2,
											...
											try {
												...
												if (updateFlag_tDBOutput_2 == 0) {
													...
													if (marche.datenotification != null) {
														pstmtInsert_tDBOutput_2.setTimestamp(4, new java.sql.Timestamp(
																marche.datenotification.getTime()));
													} else {
													...
													if (marche.lieuexecutiontypecode == null) {
														...
														pstmtInsert_tDBOutput_2.setString(8,
																marche.lieuexecutiontypecode);
													}

													if (marche.codedepartementexecution == null) {
														pstmtInsert_tDBOutput_2.setNull(9, java.sql.Types.FLOAT);
													} else {
														pstmtInsert_tDBOutput_2.setFloat(9,
																marche.codedepartementexecution);
													}

													if (marche.coderegionexecution == null) {
														pstmtInsert_tDBOutput_2.setNull(10, java.sql.Types.FLOAT);
													} else {
														pstmtInsert_tDBOutput_2.setFloat(10,
																marche.coderegionexecution);
													}

													if (marche.libelleregionexecution == null) {
														pstmtInsert_tDBOutput_2.setNull(11, java.sql.Types.VARCHAR);
													} else {
														pstmtInsert_tDBOutput_2.setString(11,
																marche.libelleregionexecution);
													}
										...
										if (doublon_marche != null) {
											...
											if (doublon_marche.datenotification != null) {
												sb_tFileOutputDelimited_2.append(FormatterUtils
														.format_Date(doublon_marche.datenotification, "yyyy-MM-dd"));
											}
											sb_tFileOutputDelimited_2.append(OUT_DELIM_tFileOutputDelimited_2);
...
// Start of branch "dim_etablissement"
									if (dim_etablissement != null) {
										...
										if (tHash_Lookup_row7 != null && tHash_Lookup_row7.getCount(row7HashKey) > 1) { // G
											...
											// System.out.println("WARNING: UNIQUE MATCH is configured for the lookup
											// 'row7' and it contains more one result from keys : row7.siren = '" +
											// row7HashKey.siren + "'");
										} // G 071
										...
										{ // start of Var scope
											...
											Var.isNotNull = DataProcessing
													.checkForNull(dim_etablissement.sirenetablissement);// ###############################
											// ###############################
											// # Output tables
...
// Start of branch "etablissement"
										if (etablissement != null) {
											...
											if (etablissement.categorieetablissement == null) {
												...
												pstmtUpdate_tDBOutput_3.setString(3,
														etablissement.categorieetablissement);
											}
											...
											if (etablissement.codecommuneetablissement == null) {
												...
												pstmtUpdate_tDBOutput_3.setString(5,
											...
											if (etablissement.communeetablissement == null) {
												...
												pstmtUpdate_tDBOutput_3.setString(6,
														etablissement.communeetablissement);
											}

											if (etablissement.departementetablissement == null) {
												pstmtUpdate_tDBOutput_3.setNull(7, java.sql.Types.VARCHAR);
											} else {
												pstmtUpdate_tDBOutput_3.setString(7,
											...
											if (etablissement.coderegionetablissement == null) {
												...
												pstmtUpdate_tDBOutput_3.setInt(9,
											...
											if (etablissement.libelleregionetablissement == null) {
												...
												pstmtUpdate_tDBOutput_3.setString(10,
											...
											try {
												...
												if (updateFlag_tDBOutput_3 == 0) {
													...
													if (etablissement.categorieetablissement == null) {
														...
														pstmtInsert_tDBOutput_3.setString(4,
																etablissement.categorieetablissement);
													}
													...
													if (etablissement.communeetablissement == null) {
														...
														pstmtInsert_tDBOutput_3.setString(7,
																etablissement.communeetablissement);
													}
													...
													if (etablissement.coderegionetablissement == null) {
														...
														pstmtInsert_tDBOutput_3.setInt(10,
																etablissement.coderegionetablissement);
													}
...
// Start of branch "row6"
											if (row6 != null) {
												...
												if (row10.sirenetablissement != null) {
													sb_tFileOutputDelimited_4.append(row10.sirenetablissement);
												}
												sb_tFileOutputDelimited_4.append(OUT_DELIM_tFileOutputDelimited_4);
												if (row10.siretetablissement != null) {
													sb_tFileOutputDelimited_4.append(row10.siretetablissement);
												}
												sb_tFileOutputDelimited_4.append(OUT_DELIM_tFileOutputDelimited_4);
												if (row10.siretetablissementvalide != null) {
													sb_tFileOutputDelimited_4.append(row10.siretetablissementvalide);
												}
												sb_tFileOutputDelimited_4.append(OUT_DELIM_tFileOutputDelimited_4);
												if (row10.categorieetablissement != null) {
													sb_tFileOutputDelimited_4.append(row10.categorieetablissement);
												}
												sb_tFileOutputDelimited_4.append(OUT_DELIM_tFileOutputDelimited_4);
												if (row10.denominationsocialeetablissement != null) {
													sb_tFileOutputDelimited_4
															.append(row10.denominationsocialeetablissement);
												}
												sb_tFileOutputDelimited_4.append(OUT_DELIM_tFileOutputDelimited_4);
												if (row10.codecommuneetablissement != null) {
													sb_tFileOutputDelimited_4.append(row10.codecommuneetablissement);
												}
												sb_tFileOutputDelimited_4.append(OUT_DELIM_tFileOutputDelimited_4);
												if (row10.communeetablissement != null) {
													sb_tFileOutputDelimited_4.append(row10.communeetablissement);
												}
												sb_tFileOutputDelimited_4.append(OUT_DELIM_tFileOutputDelimited_4);
												if (row10.departementetablissement != null) {
													sb_tFileOutputDelimited_4.append(row10.departementetablissement);
												}
												sb_tFileOutputDelimited_4.append(OUT_DELIM_tFileOutputDelimited_4);
												if (row10.libelledepartementetablissement != null) {
													sb_tFileOutputDelimited_4
															.append(row10.libelledepartementetablissement);
												}
												sb_tFileOutputDelimited_4.append(OUT_DELIM_tFileOutputDelimited_4);
												if (row10.coderegionetablissement != null) {
													sb_tFileOutputDelimited_4.append(row10.coderegionetablissement);
												}
												sb_tFileOutputDelimited_4.append(OUT_DELIM_tFileOutputDelimited_4);
												if (row10.libelleregionetablissement != null) {
													sb_tFileOutputDelimited_4.append(row10.libelleregionetablissement);
												}
												sb_tFileOutputDelimited_4.append(OUT_DELIM_tFileOutputDelimited_4);
												if (row10.codenaf != null) {
													sb_tFileOutputDelimited_4.append(row10.codenaf);
												}
												sb_tFileOutputDelimited_4
										...
										if (doublon_etablissement != null) {
											...
											if (doublon_etablissement.sirenetablissement != null) {
												sb_tFileOutputDelimited_3
														.append(doublon_etablissement.sirenetablissement);
											}
											sb_tFileOutputDelimited_3.append(OUT_DELIM_tFileOutputDelimited_3);
											if (doublon_etablissement.siretetablissement != null) {
												sb_tFileOutputDelimited_3
														.append(doublon_etablissement.siretetablissement);
											}
											sb_tFileOutputDelimited_3.append(OUT_DELIM_tFileOutputDelimited_3);
											if (doublon_etablissement.siretetablissementvalide != null) {
												sb_tFileOutputDelimited_3
														.append(doublon_etablissement.siretetablissementvalide);
											}
											sb_tFileOutputDelimited_3.append(OUT_DELIM_tFileOutputDelimited_3);
											if (doublon_etablissement.categorieetablissement != null) {
												sb_tFileOutputDelimited_3
														.append(doublon_etablissement.categorieetablissement);
											}
											sb_tFileOutputDelimited_3.append(OUT_DELIM_tFileOutputDelimited_3);
											...
											if (doublon_etablissement.codecommuneetablissement != null) {
												sb_tFileOutputDelimited_3
														.append(doublon_etablissement.codecommuneetablissement);
											}
											sb_tFileOutputDelimited_3.append(OUT_DELIM_tFileOutputDelimited_3);
											if (doublon_etablissement.communeetablissement != null) {
												sb_tFileOutputDelimited_3
														.append(doublon_etablissement.communeetablissement);
											}
											sb_tFileOutputDelimited_3.append(OUT_DELIM_tFileOutputDelimited_3);
											if (doublon_etablissement.departementetablissement != null) {
												sb_tFileOutputDelimited_3
														.append(doublon_etablissement.departementetablissement);
											}
											sb_tFileOutputDelimited_3.append(OUT_DELIM_tFileOutputDelimited_3);
										...
										if (siren_null != null) {
											...
											if (siren_null.denominationsocialeetablissement != null) {
												sb_tFileOutputDelimited_5
														.append(siren_null.denominationsocialeetablissement);
											}
											sb_tFileOutputDelimited_5.append(OUT_DELIM_tFileOutputDelimited_5);
											...
											if (siren_null.libelledepartementetablissement != null) {
												sb_tFileOutputDelimited_5
														.append(siren_null.libelledepartementetablissement);
											}
											sb_tFileOutputDelimited_5.append(OUT_DELIM_tFileOutputDelimited_5);
...
// ###############################      
					...
					if (!((Object) (context.logs_path + "/2022_daily_staging_loading/"
							+ ((int) TalendDate.getPartOfDate("MONTH", TalendDate.getCurrentDate()) + 1) + "/"
							+ TalendDate.getPartOfDate("DAY_OF_MONTH", TalendDate.getCurrentDate())
							+ "_to_dwh.csv") instanceof java.io.InputStream)) {
...
// ###############################
				...
				try {
					for (LogCatcherUtils.LogCatcherMessage lcm : tLogCatcher_1.getMessages()) {
						...
						row12.origin = (lcm.getOrigin() == null || lcm.getOrigin().length() < 1 ? null
								: lcm.getOrigin());
...
// # Output table : 'logs'
							logs_tmp.message = row12.message;
							logs = logs_tmp;
// ###############################
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final daily_loading_dwh daily_loading_dwhClass = new daily_loading_dwh();

		int exitCode = daily_loading_dwhClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = daily_loading_dwh.class.getClassLoader()
					.getResourceAsStream("cityviz/daily_loading_dwh_1_0/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = daily_loading_dwh.class.getClassLoader()
		...
		} catch (TalendException e_tWarn_6) {
			globalMap.put("tWarn_6_SUBPROCESS_STATE", -1);

			e_tWarn_6.printStackTrace();
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : daily_loading_dwh");
		}
...
/************************************************************************************************
 * 515002 characters generated by Talend Open Studio for Data Integration on the
 * 20 avril 2022 à 11:09:20 CEST
 ************************************************************************************************/